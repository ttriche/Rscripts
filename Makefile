...
doc:
	R --slave --vanilla -e 'library(roxygen2); roxygenise()'
	-git add --all man/*.Rd

test:
	R CMD INSTALL --install-tests .
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "Rscripts", "tests")); system.time(test_check("Rscripts", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran Rscripts_$(VERSION).tar.gz
	rm -rf Rscripts.Rcheck/
